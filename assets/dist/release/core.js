...
(function ($) {
  ...
  Ecsgroup.dist = '/assets/dist/release';
  ...
  // Function
  Ecsgroup.isWishList = false;
  Ecsgroup.isCountDown = false;
  Ecsgroup.isCompare = false;
  // $ = jQuery;
  /**
  ...
  Ecsgroup.mq = function (fn, mq) {
    if (document.documentElement.clientWidth > mq) {
      if (typeof fn == 'function') fn();else fn;
    }
  };
  ...
  Ecsgroup.requestInterval = function (fn, step, timeOut) {
    var handler = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame;
    if (!handler) {
      if (!timeOut) return setTimeout(fn, timeOut);else return setInterval(fn, step);
    }
    var start,
      last,
      rt = new Object();
  ...
  Ecsgroup.backToUrl = function (selector) {
    Ecsgroup.$body.on('click', selector, function (e) {
      ...
      if (!document.referrer && /Ecsgroup.domain/.test(document.referrer) == true) location.href = prevUrl;else location.href = '/';
    });
  };
   ...
   * @param {boolean} force
   */
  Ecsgroup.lazyLoad = function (selector, force) {
    var classremove = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'lazyload';
    var classadd = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : ' ls-is-cached lazyloaded';
    function load() {
...
(function ($) {
  ...
  Ecsgroup.init = function () {
    ...
    // Core
    Ecsgroup.stickyContent('.sticky-header-mobile', {
      // Initialize Sticky Header Mobile
      minWidth: 0,
      maxWidth: 768
    });
