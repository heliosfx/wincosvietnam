        ...
        success: function success(result) {
          ...
          if (!result.Ok) {
            ...
            Ecsgroup.Minipopup.open({
              ...
              template: '<div class="minipopup-box {{productClass}}">' + '<div class="minipopup-body">' + '<div class="minipopup-content">{{message}}</div>' + '</div>' + '</div>'
            });
          }
          return false;
        },
...
function initCompare(selector) {
  ...
  if (compareList.length > 0 && $(selector + ' .btn-compare').length > 0) {
    compareList.forEach(function (item) {
      $(selector + ' .btn-compare[data-id="' + item.id + '"]').removeClass('ecs-icon-compare').addClass('added ecs-icon-check-solid').attr('href', Ecsgroup.linkCompare);
    });
  } else {
    $(selector + ' .btn-compare').removeClass('added ecs-icon-check-solid').addClass('ecs-icon-compare').attr('href', 'javascript:');
  }
...
function updateWishLst(selector, productId) {
  ...
  var $this = $(selector);
  var wishListUpdate = wishList.find(function (elem, index) {
  ...
  $.ajax({
    ...
    type: 'POST',
    data: {
      listId: JSON.stringify(wishList)
    },
    beforeSend: function beforeSend() {
      $this.addClass('load-more-overlay loading');
    },
    success: function success(result) {
      if (!result.Ok) {
        ...
        $this.removeClass('load-more-overlay loading');
        $('body .btn-wishlist[data-id="' + productId + '"]').removeClass('ecs-icon-heart').addClass('added ecs-icon-heart-full').attr('href', Ecsgroup.linkWishList);
        Ecsgroup.Minipopup.open({
          productClass: ' success minipopup-center',
          message: '<p><i class="demo-icon cus-ok-circled"></i>' + result.Msg + '</p>',
          template: '<div class="minipopup-box {{productClass}}">' + '<div class="minipopup-body">' + '<div class="minipopup-content">{{message}}</div>' + '</div>' + '</div>'
...
function initHeart(selector) {
  ...
  if (wishList.length > 0 && $(selector + ' .btn-wishlist').length > 0) {
    wishList.forEach(function (value) {
      $(selector + ' .btn-wishlist[data-id="' + value + '"]').removeClass('ecs-icon-heart').addClass('added ecs-icon-heart-full').attr('href', Ecsgroup.linkWishList);
    });
...
function selectSku(selector, viewname) {
  ...
  $.ajax({
    ...
    beforeSend: function beforeSend() {
      if (htmlresult != '#product-popup-ajax') $(htmlresult).addClass('preload-container').append(preloadTemplate);else $(htmlresult).addClass('shimmer-container');
    },
    success: function success(result) {
      if (htmlresult != '#product-popup-ajax') $(htmlresult).removeClass('preload-container').remove('.pre-load');else $(htmlresult).removeClass('shimmer-container');
      if (!result.Ok) {
        $('#error-modal p').text(result.Msg);
        Ecsgroup.popup([{
          src: '#error-modal',
    ...
    success: function success(result) {
      ...
      if (!result.Ok) {
        ...
        $('.url-ajax').each(function () {
          if ($(this).data('url')) $(this).attr('href', $(this).data('url') + window.location.href);else $(this).attr('href', window.location.href);
        });
        window.history.pushState({
          state: urlbuilder,
          rand: Math.random()
        }, document.title, urlbuilder);
        setTimeout(function () {
    ...
    success: function success(result) {
      ...
      if (!result.Ok) {
        ...
        if (result.Data.listedprice != 0) {
          html = '<ins class="new-price price-promoprice">' + result.Data.price.toLocaleString('vi-VN', {
            style: 'currency',
            currency: 'VND'
          }) + '</ins>' + '<label class="product-label label-discount"><span>-' + result.Data.percent + '%</span></label>';
        } else {
          html = '<ins class="new-price">' + result.Data.price.toLocaleString('vi-VN', {
            style: 'currency',
            currency: 'VND'
          }) + '</ins>';
        }
...
function showPosition(position) {
  ...
  if ($('.select-district li').length > 1) $('.select-district li[data-value="0"]').trigger('click');else refreshFilterStore();
}
  ...
  $.ajax({
    ...
    success: function success(result) {
      if (result.CurrentPage > result.TotalPageCount) {
        ...
        window.removeEventListener('scroll', loadmoreSrcoll, {
          passive: true
        });
      } else {
...
function validateForm(btn, idform) {
  var layout = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'default';
  var submitted = true,
    formError = $(idform).find('.error-lst');
  var form = $(idform).validate({
    focusCleanup: true,
    ...
    invalidHandler: function invalidHandler(event, validator) {
      ...
      switch (layout) {
        case 'mini':
          ...
          break;
        default:
          if (validator.numberOfInvalids()) formError.show();else formError.hide();
      }
      submitted = true;
    },
    submitHandler: function submitHandler(form) {
        ...
        success: function success(result) {
          ...
          switch (layout) {
            case 'mini':
              ...
              break;
            default:
              if (!result.Ok) formError.html(errorTemplate.replace('ERROR_MSG', result.Msg)).show();else {
                sendMail(result.Data.id, '/aj/Shared/SendEmail');
                formError.hide();
...
function validateFormApi(btn, idform, source) {
  var submitted = true,
    ...
    exelApi = 'https://script.google.com/macros/s/AKfycbytuo2Pld01GVJjZrvlAMywFd6UJsN5Yy9-6SI90tGS9m1nR6aDhOy3yHJOppdo9QEHUg/exec',
